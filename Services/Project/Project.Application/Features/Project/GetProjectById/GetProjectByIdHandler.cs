        ...
        : IQueryHandler<GetProjectByIdRequest, ApiResponse<GetProjectByIdResponse>>
    {
        public async Task<ApiResponse<GetProjectByIdResponse>> Handle(GetProjectByIdRequest request, CancellationToken cancellationToken)
        {
            ...
            var getProjectByIdResponse = new GetProjectByIdResponse()
            {
                ...
                UserCount = userCount,
                FolderCount = folderRepository.GetAllQueryAble().Where(e => e.ProjectId == request.Id && e.IsDeleted == false).Count(),
                FileCount = fileRepository.GetAllQueryAble().Where(e => e.ProjectId == request.Id && e.IsDeleted == false).Count(),
                Size = (double)fileRepository.GetAllQueryAble().Where(e => e.ProjectId == request.Id && e.IsDeleted == false).Sum(e => e.Size),
